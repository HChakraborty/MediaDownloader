...
namespace Downloader.Business
{
    public class VideoDownloader
    {
        ...
        public VideoDownloader(IOptions<BaseAppSettings> appsettings, IWebHostEnvironment env)
        {
            ...
            _ytdl.YoutubeDLPath = Path.Combine(_baseDir, _appSettings.PackageDirectory, _appSettings.YtDlpPath);
            _ytdl.FFmpegPath = Path.Combine(_baseDir, _appSettings.PackageDirectory, _appSettings.FFmpegPath);
            _ytdl.OutputFolder = Path.Combine(_baseDir, _appSettings.OutputFolder, DateTime.Now.ToString("yyyyMMddHHmmss"));
        }
        ...
        private async Task<VideoFile> GetVideoFromSource(string videoUrl)
        {
            ...
            RunResult<string> videoFile = await _ytdl.RunVideoDownload(Uri.UnescapeDataString(videoUrl), overrideOptions: options);
        ...
        {
            RunResult<VideoData> videoDetails = await _ytdl.RunVideoDataFetch(Uri.UnescapeDataString(videoUrl));
        ...
        private void DeleteOutputFolder()
        {
            ...
            {
                throw new Exception("You are trying to delete a directory or its contents which doesn't exist", ex);
            }
        }
    }
}
